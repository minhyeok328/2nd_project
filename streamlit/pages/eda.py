...
Updated:
- 2026-03-16: pages용 구조 및 이미지 경로 수정 (@nobrain711)
=========================================================================
"""
...
EDA_IMAGE_PATHS = {
    "eda1": "assets/eda/eda1.png",
    "eda2": "assets/eda/eda2.png",
    "eda3": "assets/eda/eda3.png",
    "eda4": "assets/eda/eda4.png",
    "eda5": "assets/eda/eda5.png",
    "eda6": "assets/eda/eda6.png",
}

COLUMN_WIDTHS = [1, 5, 1, 5, 1]
...
def render_eda_page() -> None:
    ...
    with st.container(border=True):
        _, sec_a, _, sec_b, _ = st.columns(COLUMN_WIDTHS, gap="large")
        with sec_a:
            st.image(
                EDA_IMAGE_PATHS["eda1"],
                caption="Correlation Heatmap with Attrition_Flag",
                width="stretch",
            )
        with sec_b:
            st.markdown(
    ...
    with st.container(border=True):
        ...
        with sec_b:
            st.image(
                EDA_IMAGE_PATHS["eda2"],
                caption="Distribution of Customer Churn",
                width="stretch",
            )
    ...
    with st.container(border=True):
        ...
        with sec_a:
            st.image(
                EDA_IMAGE_PATHS["eda3"],
                caption="사용 비율에 따른 이탈 밀도도",
                width="stretch",
            )
        with sec_b:
            st.markdown(
    ...
    with st.container(border=True):
        ...
        with sec_b:
            st.image(
                EDA_IMAGE_PATHS["eda4"],
                caption="이탈 여부에 따른 결제 횟수 차이",
                width="stretch",
            )
    ...
    with st.container(border=True):
        ...
        with sec_a:
            st.image(
                EDA_IMAGE_PATHS["eda5"],
                caption="소득 수준별 이탈 고객 분포",
                width="stretch",
            )
        with sec_b:
            st.markdown(
    ...
    with st.container(border=True):
        ...
        with sec_b:
            st.image(
                EDA_IMAGE_PATHS["eda6"],
                caption="학력과 소득 수준의 관계",
                width="stretch",
            )
